...
import main.java.embl.rieslab.emu.uiexamples.htsmlm.autopilot.utils.math.Percentile;
...
public class ImageBrightnessFilterFocusFinder implements
	 ...
	 * @return factory
	 */
	public static final FocusFinderFactoryInterface<Double> getFactory(
			final FocusFinderFactoryInterface<Double> pFocusFinderFactory,
		...
		final FocusFinderFactoryInterface<Double> lFactory = new FocusFinderFactoryInterface<Double>() {
			...
			public FocusFinderInterface<Double> instantiate() {
				final FocusFinderInterface<Double> lFocusFinder = pFocusFinderFactory
						.instantiate();
	...
	/**
	 * Constructs an image brightnes filter focus finder given a focus finder
	 * and normalized average brightness percentile.
	 * 
	 * @param pFocusFinder
	 *            focus finder
	 * @param pImageNormalizedAverageBrightnessPercentile
	 *            normalized average brightness percentile
	 */
	public ImageBrightnessFilterFocusFinder(
			final FocusFinderInterface<Double> pFocusFinder,
			final double pImageNormalizedAverageBrightnessPercentile) {
		super();
	...
	public double addImage(final Double pPosition,
		...
		mImagePositionToAverageBrightnessMap.put(pPosition, lAverageBrightness);
	...
	private double computeAverageBrightness(
			final DoubleArrayImage pDoubleArrayImage) {
		final double lAverageBrightness = pDoubleArrayImage.normL1()
	...
	private void ensureUpToDate() {
		if (!mUpToDate) {
			...
			final double[] lPositions = mImagePositionToAverageBrightnessMap
					.keys();

			for (final double lPosition : lPositions) {
				final double lAverageBrightness = mImagePositionToAverageBrightnessMap
						.get(lPosition);
				...
				if (isBrightEnough(lNormalizedBrightness)) {
					if (mDebug) {
						System.out
								.format("accepted image at: %g because average brightness (nab=%g) high enough \n",
										lPosition, lNormalizedBrightness);
					}
				} else {
					mFocusFinder.removeImage(lPosition);
					if (mDebug) {
						System.out
								.format("rejected image at: %g because average brightness (nab=%g) too low! \n",
										lPosition, lNormalizedBrightness);
					}
	...
	private void computeNormalizationParametersAndPercentile() {
		final double[] lAverageBrightnessArray = mImagePositionToAverageBrightnessMap
				.values();
	...
	private final double getNormalizedAverageBrightness(
			final double pAverageBrightness) {
		final double lNormalizedAverageBrightness = mFactor
				* (pAverageBrightness - mOffset);
