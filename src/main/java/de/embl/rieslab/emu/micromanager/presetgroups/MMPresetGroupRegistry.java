...
public class MMPresetGroupRegistry {
	...
	private HashMap<String, MMPresetGroup> groups_;
		...
		if(groups != null){
			for(int i=0;i<groups.size();i++){
				...
				try {
					...
					groups_.put(groups.get(i), new MMPresetGroup(groups.get(i),core_.getAvailableConfigs(groups.get(i)),affectedmmprops));
