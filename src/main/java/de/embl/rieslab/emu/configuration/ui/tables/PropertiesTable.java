...
public class PropertiesTable extends JPanel {
	...
	private void updateHelper(int row){
		...
		if(s.matches(MultiStateUIProperty.getGenericStateName())){
			...
			help_.update(s,"Set the device property value corresponding to the state number "+diff+". The allowed values can be read out from the device property browser. State values cannot be left unconfigured.");
		} else if (s.contains(TwoStateUIProperty.getOnStateLabel())){
			s = (String) table.getValueAt(row-1, 0);
			help_.update(s,"Enter the device property value corresponding to the ON state. The allowed values can be read out from the device property browser. State values cannot be left unconfigured.");
		} else if (s.contains(TwoStateUIProperty.getOffStateLabel())){
			s = (String) table.getValueAt(row-2, 0);
			help_.update(s,"Enter the device property value corresponding to the OFF state. The allowed values can be read out from the device property browser. State values cannot be left unconfigured.");
